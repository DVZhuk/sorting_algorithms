...
selectionButton.addEventListener('click', function () {
  ...
  let forI = setTimeout(function request() {
    ...
    let forJ = setInterval(function(i) {
      ...
      if (j-1 > i + 1) {
        lines[j-1].style.backgroundColor = 'blue';
      ...
      if (array[j-1] < minValue) {
        ...
        let animation = lines[j-1].animate([
          {transform: 'translateX(' + i * (500 / 9) + 'px)'}
        ], 300);
        animation.addEventListener('finish', function() {
          selectionSort.insertBefore(lines[j-1], lines[i]);
          lines[i].style.transform = 'translateX(' + i * (500 / 9) + 'px)';
          if (i < array.length - 2) {
            lines[i].style.backgroundColor = 'yellow';
          } else {
